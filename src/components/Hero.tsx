...
export default function Hero() {
  ...
  return (
    <section className="h-screen flex flex-col justify-between bg-white px-6 py-12">
      ...
      {/* Rating + Botón abajo */}
      <motion.div
        className="mb-12 w-full max-w-4xl flex flex-col lg:flex-row items-center lg:justify-between gap-6 mx-auto"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        ...
        <Link
          to={contact.url}
          className="bg-black text-white px-6 py-3 rounded-full font-semibold hover:bg-gray-800 transition whitespace-nowrap"
        >
          {contact.title}
        </Link>
      </motion.div>
